...
### v1.0.12
- feat: App 图标改为简约线条风格鸽子（矢量绘制，圆角方形蓝色渐变底），替换此前的系统 Emoji 渲染，观感更接近微信/QQ 一类的极简图标
- feat: 菜单栏图标同步改为白色线条鸽子，透明底、居中显示，并加大到与其他状态栏图标一致的尺寸
- feat: 弹出面板顶部「Tokei」旁的图标从系统 timer 符号换成同款鸽子图标
- feat: Codex 用量卡片新增「按模型」展开，按真实模型（GPT-5.5 / Deepseek V4 Pro 等）分别显示 token 量、缓存命中率与成本，和 Claude / Gemini 卡片一致（后端早已分模型统计，此前仅前端未展示）
- fix: 多设备同步改为「本机权威主机分发 usage.30s.py + rebase 全程 autostash」，修复采集脚本刷新与同步竞态导致设备时间戳卡死、既推不上去也拉不到对端数据的问题

### v1.0.11
- feat: App 图标 + 菜单栏图标改为鸽子 🕊️（直接渲染系统 Apple Color Emoji 字形，原生配色，透明背景）
